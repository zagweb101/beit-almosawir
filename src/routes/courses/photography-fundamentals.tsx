...
import {
  getPhotographyFundamentalsCourse,
  photographyFundamentalsOgImage,
} from "@/data/courses/photography-fundamentals";
import { SITE } from "@/lib/site-config";
import { useT } from "@/lib/i18n";
import { usePhotographyFundamentalsCourse } from "@/hooks/use-course-data";
...
const defaultCourse = getPhotographyFundamentalsCourse("ar");
const ogImageUrl = `${SITE.siteUrl}${photographyFundamentalsOgImage}`;
const canonicalUrl = `${SITE.siteUrl}${defaultCourse.seo.canonicalPath}`;

export const Route = createFileRoute("/courses/photography-fundamentals")({
  head: () => ({
    meta: [
  ...
  component: PhotographyFundamentalsPage,
});

function PhotographyFundamentalsPage() {
  const { lang } = useT();
  const course = usePhotographyFundamentalsCourse();
