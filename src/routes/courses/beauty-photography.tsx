...
import {
  getBeautyPhotographyCourse,
  beautyPhotographyOgImage,
} from "@/data/courses/beauty-photography";
import { SITE } from "@/lib/site-config";
import { useT } from "@/lib/i18n";
import { useBeautyPhotographyCourse } from "@/hooks/use-course-data";
...
const defaultCourse = getBeautyPhotographyCourse("ar");
const ogImageUrl = `${SITE.siteUrl}${beautyPhotographyOgImage}`;
const canonicalUrl = `${SITE.siteUrl}${defaultCourse.seo.canonicalPath}`;

export const Route = createFileRoute("/courses/beauty-photography")({
  head: () => ({
    meta: [
  ...
  component: BeautyPhotographyPage,
});

function BeautyPhotographyPage() {
  const { lang } = useT();
  const course = useBeautyPhotographyCourse();
