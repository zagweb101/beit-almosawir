...
import { getLightingMasteryCourse, lightingMasteryOgImage } from "@/data/courses/lighting-mastery";
import { SITE } from "@/lib/site-config";
import { useT } from "@/lib/i18n";
import { useLightingMasteryCourse } from "@/hooks/use-course-data";
...
const defaultCourse = getLightingMasteryCourse("ar");
const ogImageUrl = `${SITE.siteUrl}${lightingMasteryOgImage}`;
const canonicalUrl = `${SITE.siteUrl}${defaultCourse.seo.canonicalPath}`;

export const Route = createFileRoute("/courses/lighting-mastery")({
  head: () => ({
    meta: [
  ...
  component: LightingMasteryPage,
});

function LightingMasteryPage() {
  const { lang } = useT();
  const course = useLightingMasteryCourse();
